...
sensors into three independent utilities, presented as three bottom-navigation tabs in a
single `MainActivity`:
...
```
com.rama.gpsapp
├── MainActivity.kt        # Single-activity host, 3-route bottom nav
...
Two independent DataStore (Preferences) stores persist settings: `gesture_settings` and
`anti_theft_settings`. Both `GestureShortcutService` and `TheftAlarmService` are foreground
`LifecycleService`s (`foregroundServiceType="specialUse"`) restarted on boot by
`BootCompletedReceiver` if they were previously enabled/armed.
...
or `pdr/` — these packages have corresponding unit tests under `app/src/test/java/com/rama/gpsapp/`.

## Conventions

- Package-by-feature: new sensor-driven features get their own top-level package (mirroring
  `gesture/`, `theft/`, `pdr/`) with detector/engine classes, a `data/` settings model +
  repository, and a `ui/<feature>/` Compose screen + ViewModel.
- Detector classes (e.g. `ShakeDetector`, `FlipDetector`, `TwistDetector`, `StepDetector`) are
  plain Kotlin classes that consume `SensorSample`s and are unit-testable in isolation — keep
  new detection logic in this style rather than embedding it directly in services.
